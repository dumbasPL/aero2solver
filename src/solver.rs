...
impl Aero2Solver {
    pub fn new<P: AsRef<Path>>(names_file: P, config_file: P, model_file: P) -> Result<Self> {
    ...
    pub fn solve(
        ...
        let mut detections: Vec<_> = detections
            .iter()
            .flat_map(|det| {
                det.best_class(Some(min_threshold))
                    .map(|(class_index, prob)| (det, prob, &self.object_labels[class_index]))
            })
            .collect();

        if detections.len() != expected_length {
            return Err(anyhow!(
